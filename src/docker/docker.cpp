...
Future<Nothing> Docker::prepare_volume(
		const std::string& volume,
				  ...
				  const string& sandboxDirectory,
				    ...
				    const process::Subprocess::IO& stderr
				  ) const{
...
//	   net_argv.push_back(name);
	   ...
	   net_argv.push_back("sh");
	   net_argv.push_back("-c");
	   net_argv.push_back(n_cmd);
	   n_cmd = "sh";
       //string& command = "dev=`rbd map volumes/volume-959860f0-1c5a-4ea9-a2a9-0ca68e8db713`;mkfs.xfs $dev;rbd unmap $dev";
	   Try<Subprocess> net_create = subprocess(
			 n_cmd,
			 net_argv,
			 Subprocess::PATH("/dev/null"),
			 stdout,
			 ...
			 None(),
			 environment);
	   if (net_create.isError()) {
	      return Failure(net_create.error());
	    }
	   return net_create.get().status().then(lambda::bind(
		        &Docker::_run,
		        lambda::_1))
		    .onDiscard(lambda::bind(&commandDiscarded, net_create.get(), n_cmd));;
}
...
//return Failure("<<<<<Achtung!!!>>>>> No docker info found in container info");
  ...
  foreach (const Volume& volume, containerInfo.volumes()) {
    ...
    if (volume.has_host_path()) {
      ...
      //TODO(gokrokve) this is a workaround. We need to format only RBD volumes
//      //As there is no volume type passed we will detect RBD volume by its name
//      if (strings::startsWith(volume.host_path(),"volumes/")){
//         prepare_volume(volume.host_path(),"volume", sandboxDirectory, stdout, stderr);
//      }
      if (volume.has_mode()) {
        switch (volume.mode()) {
